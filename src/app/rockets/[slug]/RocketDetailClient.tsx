...
import type { Rocket as RocketType } from "@/lib/types";

interface RocketDetailClientProps {
  rocket: RocketType;
  prevRocket: { name: string; slug: string } | null;
  nextRocket: { name: string; slug: string } | null;
  allRockets: RocketType[];
}

export default function RocketDetailClient({
  rocket,
  prevRocket,
  nextRocket,
  allRockets,
}: RocketDetailClientProps) {
                ...
                {[
                  {
                    ...
                    bar:
                      (rocket.payloadToLEO /
                        Math.max(...allRockets.map((r) => r.payloadToLEO))) *
                      100,
                  },
                  {
