...
import {
  Globe2, ArrowLeft, Thermometer, Ruler, Weight,
  MoveHorizontal, Clock, Zap, Moon, Shield, Activity,
} from "lucide-react";
import { planets } from "@/lib/data";
import { MotionFadeIn } from "@/components/ui/MotionWrapper";
import { HeroImage, ScrollReveal } from "@/components/ui/PlanetDetailClient";
...
const typeColors: Record<string, string> = {
  ...
  MOON: "from-slate-500/20 to-slate-600/20 text-slate-400 border-slate-500/30",
};
...
export async function generateMetadata({
  ...
  if (!planet) return { title: "Celestial Body Not Found | SpaceAtlas" };
  return {
    title: `${planet.name} | SpaceAtlas Solar System`,
    description: planet.description.slice(0, 160),
    openGraph: {
      title: `${planet.name} — ${planet.type.replace("_", " ")}`,
      description: planet.description.slice(0, 160),
      images: planet.imageUrl ? [planet.imageUrl] : [],
    },
  };
}
    ...
    <div className="relative bg-[#020617] min-h-screen font-light selection:bg-[#38bdf8]/30">
      ...
      <section className="relative min-h-[80vh] flex flex-col justify-end pb-24 overflow-hidden border-b border-space-500/30">
        <div className="absolute inset-0 z-0 bg-black">
          <HeroImage src={planet.imageUrl || "/placeholder.jpg"} alt={planet.name} />
