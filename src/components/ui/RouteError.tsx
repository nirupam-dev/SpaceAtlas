...
interface RouteErrorProps {
  ...
  reset: () => void;
  /** Display name for the section, e.g. "Rockets" */
  section: string;
}

/**
 * Shared error UI used by every route-level `error.tsx`.
 * Logs the error and renders a branded recovery screen.
 */
export default function RouteError({ error, reset, section }: RouteErrorProps) {
  const log = createLogger(section);

  useEffect(() => {
    log.error(`Unhandled error in ${section}`, {
